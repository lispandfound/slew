...
import Brick
    ( attrName,
      ...
      withAttr,
      Widget,
      Padding(Pad, Max), padBottom, padLeftRight )
...
import Model.AppState
    ( AppState,
      ...
      filterJobs )
import Model.Job
    ( Job(stateReason, account, jobState, jobId, name, userName,
          partition, nodes, cpus, memoryPerNode, timeLimit, startTime,
          endTime, exitCode),
      showWith,
      ExitCode(status) )
...
formatTime pico =
  let
    ...
    secs = seconds `mod` 60
  in "" +| padLeftF 2 '0' hours |+ ":" +|  padLeftF 2 '0' minutes |+ ":" +| padLeftF 2 '0' secs |+ ""
