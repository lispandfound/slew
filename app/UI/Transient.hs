...
drawTransientView menu = go (tree menu)
    where childLabel menu = withAttr (menu ^. style) (str [menu ^. char, ':']) <+> padLeftRight 1 (txt (menu ^. name))
          go current = vBox [
...
handleTransientEvent (V.EvKey V.KEsc []) = gets shouldClose <* modify goUp
  where shouldClose st = if isRoot st then pure Close else pure Up
